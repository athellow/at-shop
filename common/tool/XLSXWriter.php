...
class XLSXWriter
{
	...
	public function __construct()
	{
		...
		$this->addCellStyle($number_format='GENERAL', $style_string=null);
		$this->addCellStyle($number_format='GENERAL', $style_string=null);
		$this->tempdir = \Yii::getAlias('@frontend/runtime');
		$this->rmTempdirFile();
	}
	...
	protected function rmTempdirFile()
	{
		$result = true;
		if(! is_dir($this->tempdir)){
			$result = mkdir($this->tempdir);
		}
		$prefix = 'xlsx_writer_';
		if ($result && ($handle = opendir($this->tempdir))){
			while(false !== ($file = readdir($handle))) {
				$compareTime = time() - 6*3600;
				$tmpFile = $this->tempdir . DIRECTORY_SEPARATOR . $file;
				if(substr($file, 0, strlen($prefix)) == $prefix && $compareTime >= filemtime($tmpFile)){
					@unlink($tmpFile);
				}
			}
			closedir($handle);
	...
	public function delete()
    {
        foreach($this->sheets as $sheet_name => $sheet) {
            try{
                if (file_exists($sheet->filename)) {
                    @unlink($sheet->filename);
                }
                $sheet->file_writer->close();
                unset($sheet->file_writer);
            }catch (\Exception $e){
                continue;
            }
        }
